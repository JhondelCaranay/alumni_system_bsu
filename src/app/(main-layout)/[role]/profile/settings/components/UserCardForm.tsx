...
const UserCard: React.FC<UserCardProps> = ({ data }) => {
  ...
  const updateProfile = useMutateProcessor<
    UpdateUserSchemaType,
    UserWithProfile
  >(`/users/${data?.id}`, null, "PATCH", ["users", data?.id], {
      ...
      >
        ...
        <div className="flex flex-col ml-2 justify-between h-full">
          <div className="flex flex-col gap-y-1">
            <span className="text-md font-semibold text-zinc-600 dark:text-white">
              {data?.name}
            </span>
            <span className="text-sm font-semibold text-zinc-600 dark:text-white capitalize">
